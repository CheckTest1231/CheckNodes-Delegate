...

{
    ...
    0 */2 * * * sleep 10 && /root/go/bin/lavad q staking validator $(/root/go/bin/lavad keys show wallet --bech val -a) | grep -E "tokens" >> /root/AUTODELEGATE/Lava.txt
    0 */2 * * * sleep 30 && /root/go/bin/zetacored q staking validator $(/root/go/bin/zetacored keys show wallet --bech val -a) | grep -E "tokens" >> /root/AUTODELEGATE/Zeta.txt
    0 */2 * * * sleep 50 && /root/go/bin/dymd q staking validator $(/root/go/bin/dymd keys show wallet --bech val -a) | grep -E "tokens" >> /root/AUTODELEGATE/Dymension.txt
} | crontab -
